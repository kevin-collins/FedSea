...
from common.model_ops.ranking import losses_impl
...
def make_loss_fn(loss_keys,
    ...
    """
    if reduction not in tf.losses.Reduction.all() or reduction == tf.losses.Reduction.NONE:
...
def _pairwise_hinge_loss(
    ...
    with tf.name_scope(loss.name, 'pairwise_hinge_loss',
                       (labels, logits, weights)):
...
def _pairwise_logistic_loss(
    ...
    with tf.name_scope(loss.name, 'pairwise_logistic_loss',
                       (labels, logits, weights)):
        return loss.compute(labels, logits, weights, reduction)
...
def _pairwise_soft_zero_one_loss(
    ...
    with tf.name_scope(loss.name, 'pairwise_soft_zero_one_loss',
                       (labels, logits, weights)):
        return loss.compute(labels, logits, weights, reduction)
...
def _softmax_loss(
    ...
    with tf.name_scope(loss.name, 'softmax_loss',
                       (labels, logits, weights)):
        return loss.compute(labels, logits, weights, reduction)
...
def _sigmoid_cross_entropy_loss(
    ...
    with tf.name_scope(loss.name, 'sigmoid_cross_entropy_loss',
                       (labels, logits, weights)):
        return loss.compute(labels, logits, weights, reduction)
...
def _mean_squared_loss(
    ...
    with tf.name_scope(loss.name, 'mean_squared_loss',
                       (labels, logits, weights)):
        return loss.compute(labels, logits, weights, reduction)
...
def _list_mle_loss(
    ...
    with tf.name_scope(loss.name, 'list_mle_loss',
                       (labels, logits, weights)):
        return loss.compute(labels, logits, weights, reduction)
...
def _approx_ndcg_loss(labels,
    ...
    with tf.name_scope(loss.name, 'approx_ndcg_loss',
                       (labels, logits, weights)):
        return loss.compute(labels, logits, weights, reduction)
...
def _approx_mrr_loss(labels,
    ...
    with tf.name_scope(loss.name, 'approx_mrr_loss', (labels, logits, weights)):
        return loss.compute(labels, logits, weights, reduction)
...
def _gumbel_softmax_sample(
    ...
    """
    with tf.name_scope(name, 'gumbel_softmax_sample', (labels, logits, weights)):
        ...
        expanded_labels = tf.reshape(expanded_labels, [batch_size * sample_size, list_size])
        ...
        sampled_logits = tf.where(is_label_valid,
                                  sampled_logits / temperature,
                                  tf.log(1e-20) * tf.ones_like(sampled_logits))
        sampled_logits = tf.log(tf.nn.softmax(sampled_logits) + 1e-20)
        ...
        if expanded_weights is not None:
            ...
            expanded_weights = tf.cond(pred=tf.equal(tf.rank(expanded_weights), 1),
                                       true_fn=true_fn, false_fn=false_fn)
            expanded_weights = tf.tile(expanded_weights, [1, sample_size, 1])
            expanded_weights = tf.reshape(expanded_weights, [batch_size * sample_size, -1])

        return expanded_labels, sampled_logits, expanded_weights


def _sample_gumbel(shape, eps=1e-20, seed=None):
    u = tf.random_uniform(shape, minval=0, maxval=1, dtype=tf.float32, seed=seed)
