...
from common.model_ops.ranking.sort_ops import argsort
    ...
    with tf.name_scope(name='sort_by_scores'):
        ...
        if shuffle_ties:
            shuffle_ind = _to_nd_indices(
                argsort(tf.random_uniform(tf.shape(input=scores), seed=seed), stable=True))
...
def sorted_ranks(scores, shuffle_ties=True, seed=None):
    ...
    with tf.name_scope(name='sorted_ranks'):
        ...
        ranks = argsort(sorted_positions) + 1
        return ranks
...
def organize_valid_indices(is_valid, shuffle=True, seed=None):
    ...
    with tf.name_scope(name='organize_valid_indices'):
        ...
        if shuffle:
            values = tf.random_uniform(output_shape, seed=seed)
        else:
            values = (
        ...
        indices = argsort(rand, direction='DESCENDING', stable=True)
...
def reshape_first_ndims(tensor, first_ndims, new_shape):
    ...
    if isinstance(tensor, tf.SparseTensor):
        return tf.sparse_reshape(tensor, new_shape)
...
def inverse_max_dcg(labels,
    ...
    discounted_gain = tf.reduce_sum(input_tensor=discounted_gain, axis=1, keep_dims=True)
    return tf.where(
...
def _circular_indices(size, num_valid_entries):
    ...
    """
    with tf.name_scope(name='circular_indices'):
        ...
        num_valid_entries = tf.where(
            ...
            num_valid_entries)
        batch_indices = tf.mod(batch_indices, num_valid_entries)
...
def padded_nd_indices(is_valid, shuffle=False, seed=None):
    ...
    """
    with tf.name_scope(name='nd_indices_with_padding'):
        is_valid = tf.convert_to_tensor(value=is_valid)
