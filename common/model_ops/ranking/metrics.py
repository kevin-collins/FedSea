...
from common.model_ops.ranking import metrics_impl
from common.model_ops.ranking import utils

_DEFAULT_GAIN_FN = lambda label: tf.pow(2.0, label) - 1

_DEFAULT_RANK_DISCOUNT_FN = lambda rank: tf.log(2.) / tf.log1p(rank)
...
def compute_mean(metric_key,
    ...
    Args:
        ...
        labels: A `Tensor` of the same shape as `predictions` representing relevance.
    ...
    assert metric_key in metric_dict, ('metric_key %s not supported.' % metric_key)
...
def make_ranking_metric_fn(metric_key,
    ...
    Args:
        metric_key: A key in `RankingMetricKey`.
        weights_feature_name: A `string` specifying the name of the weights feature in `features` dict.
        topn: An `integer` specifying the cutoff of how many items are considered in the metric.
        name: A `string` used as the name for this metric.
...
def mean_reciprocal_rank(labels,
    ...
    Args:
        ...
        predictions: A `Tensor` with shape [batch_size, list_size]. Each value is the ranking score of the corresponding example.
        weights: A `Tensor` of the same shape of predictions or [batch_size, 1]. The former case is per-example and the latter case is per-list.
        topn: An integer cutoff specifying how many examples to consider for this metric. If None, the whole list is considered.
        name: A string used as the name for this metric.
    ...
    with tf.name_scope(metric.name, 'mean_reciprocal_rank', (labels, predictions, weights)):
        mrr, per_list_weights = metric.compute(labels, predictions, weights)
        return tf.metrics.mean(mrr, per_list_weights)
...
def average_relevance_position(labels, predictions, weights=None, name=None):
    ...
    with tf.name_scope(metric.name, 'average_relevance_position', (labels, predictions, weights)):
        arp, per_list_weights = metric.compute(labels, predictions, weights)
    return tf.metrics.mean(arp, per_list_weights)
...
def precision(labels, predictions, weights=None, topn=None, name=None):
    ...
    with tf.name_scope(metric.name, 'precision', (labels, predictions, weights)):
        precision_at_k, per_list_weights = metric.compute(labels, predictions,
                                                          weights)
    return tf.metrics.mean(precision_at_k, per_list_weights)
...
def mean_average_precision(labels,
    ...
    with tf.name_scope(metric.name, 'mean_average_precision', (labels, predictions, weights)):
        per_list_map, per_list_weights = metric.compute(labels, predictions,
                                                        weights)
    return tf.metrics.mean(per_list_map, per_list_weights)
...
def normalized_discounted_cumulative_gain(
    ...
    Args:
        ...
        topn: A cutoff for how many examples to consider for this metric.
        name: A string used as the name for this metric.
        gain_fn: (function) Transforms labels. Note that this implementation of NDCG assumes that this function is *increasing* as a function of its imput.
        rank_discount_fn: (function) The rank discount function. 
            Note that this implementation of NDCG assumes that this function is *decreasing* as a function of its imput.
    ...
    with tf.name_scope(metric.name, 'normalized_discounted_cumulative_gain', (labels, predictions, weights)):
...
def discounted_cumulative_gain(labels,
    ...
    with tf.name_scope(name, 'discounted_cumulative_gain', (labels, predictions, weights)):
        dcg, per_list_weights = metric.compute(labels, predictions, weights)
    return tf.metrics.mean(dcg, per_list_weights)
...
def ordered_pair_accuracy(labels, predictions, weights=None, name=None):
    ...
    with tf.name_scope(metric.name, 'ordered_pair_accuracy', (labels, predictions, weights)):
...
def eval_metric(metric_fn, **kwargs):
    ...
    Args:
        ...
        **kwargs: A collection of argument values to be passed to the metric, e.g.
            labels and predictions. See `_RankingMetric` and the various metric definitions in tfr.metrics for the specifics.
  
    Returns:
        The evaluation of the metric on the input ranked lists.
  
    Raises:
        ValueError: One of the arguments required by the metric is not provided in the list of arguments included in kwargs.
