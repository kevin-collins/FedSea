...
def _per_example_weights_to_per_list_weights(weights, relevance):
    ...
    Such a computation is good for the following scenarios:
        - When all the weights are 1.0, the per list weights will be 1.0 everywhere,
            even for lists without any relevant examples because sum(per_list_weights) ==  num(sum(relevance) != 0)
            This handles the standard ranking metrics where the weights are all 1.0.
        - When every list has a nonzero weight, the default weight is not used. This handles the unbiased metrics well.
    ...
    per_list_relevance = tf.reduce_sum(
        input_tensor=relevance, axis=1, keep_dims=True)
    nonzero_relevance = tf.cast(tf.greater(per_list_relevance, 0.0), tf.float32)
    nonzero_relevance_count = tf.reduce_sum(
        input_tensor=nonzero_relevance, axis=0, keep_dims=True)

    per_list_weights = tf.divide(
        tf.reduce_sum(input_tensor=weights * relevance, axis=1, keep_dims=True),
        per_list_relevance)
    sum_weights = tf.reduce_sum(
        input_tensor=per_list_weights, axis=0, keep_dims=True)

    avg_weight = tf.divide(sum_weights, nonzero_relevance_count)
    return tf.where(
...
def _discounted_cumulative_gain(labels,
    ...
    Args:
        labels: The relevance `Tensor` of shape [batch_size, list_size]. For the ideal ranking, the examples are sorted by relevance in reverse order.
        weights: A `Tensor` of the same shape as labels or [batch_size, 1]. The former case is per-example and the latter case is per-list.
        gain_fn: (function) Transforms labels.
        rank_discount_fn: (function) The rank discount function.

    Returns:
        A `Tensor` as the weighted discounted cumulative gain per-list. The tensor shape is [batch_size, 1].
    """
    ...
    return tf.reduce_sum(
        input_tensor=weights * gain * discount, axis=1, keep_dims=True)
...
def _per_list_precision(labels, predictions, topn):
    ...
    Returns:
        A `Tensor` of size [batch_size, 1] containing the precision of each query respectively.
    """
    ...
    per_list_precision = tf.divide(
        tf.reduce_sum(input_tensor=relevance, axis=1, keep_dims=True),
        tf.reduce_sum(
            input_tensor=tf.ones_like(relevance), axis=1, keep_dims=True))
...
def _prepare_and_validate_params(labels, predictions, weights=None, topn=None):
    ...
    predictions = tf.where(is_label_valid,
                           predictions,
                           -1e-6 * tf.ones_like(predictions) + tf.reduce_min(input_tensor=predictions, axis=1, keep_dims=True))
...
class MRRMetric(_RankingMetric):
    ...
    def compute(self, labels, predictions, weights):
        ...
        mrr = tf.reduce_max(
            input_tensor=relevance * reciprocal_rank, axis=1, keep_dims=True)
...
class MeanAveragePrecisionMetric(_RankingMetric):
    ...
    def compute(self, labels, predictions, weights):
        ...
        per_list_precisions = tf.divide(per_list_relevant_counts, per_list_cutoffs)
        ...
        total_relevance = tf.reduce_sum(
            input_tensor=sorted_weights * sorted_relevance, axis=1, keep_dims=True)
        per_list_map = tf.divide(total_precision, total_relevance)
        # per_list_weights are computed from the whole list to avoid the problem of
        # 0 when there is no relevant example in topn.
        per_list_weights = _per_example_weights_to_per_list_weights(
            weights, tf.cast(tf.greater_equal(labels, 1.0), dtype=tf.float32))
...
class NDCGMetric(_RankingMetric):
    ...
    def compute(self, labels, predictions, weights):
        ...
        per_list_ndcg = tf.divide(dcg, ideal_dcg)
        per_list_weights = _per_example_weights_to_per_list_weights(
            weights=weights,
...
class DCGMetric(_RankingMetric):
    ...
    def compute(self, labels, predictions, weights):
        ...
        per_list_weights = _per_example_weights_to_per_list_weights(
            weights=weights,
            relevance=self._gain_fn(tf.cast(labels, dtype=tf.float32)))
        per_list_dcg = tf.divide(dcg, per_list_weights)
